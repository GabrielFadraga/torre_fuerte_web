...
from TFuerte.api.commercial_api import AdhesionContractsAPI, AdhesionSuppliersAPI

class CommercialAdhesionContractsState(rx.State):
    # Datos
    ...
    # Autocompletado de proveedores de adhesión
    adhesion_suppliers_list: List[dict] = []
    ...
    def load_data(self):
        ...
        data = AdhesionContractsAPI.get_all()
        if data:
            self.contracts_data = data
        else:
            self.contracts_data = []

        suppliers = AdhesionSuppliersAPI.get_all()
        self.adhesion_suppliers_list = suppliers if suppliers else []

        self.apply_filters()
        self.loading = False
        yield rx.toast.success(f"Contratos de adhesión cargados: {len(data)}", position="top-right")
    ...
    def add_contract(self, form_data: dict):
        ...
        result = AdhesionContractsAPI.insert(insert_data)
        if result:
    ...
    def update_contract(self, form_data: dict):
        ...
        result = AdhesionContractsAPI.update(contract_id, update_data)
        if result:
            yield from self.load_data()
    ...
    def delete_selected(self):
        if not self.selected_items:
            self.show_delete_dialog = False
            return
        success = AdhesionContractsAPI.delete(self.selected_items)
        if success:
            self.selected_items = []
            yield from self.load_data()
            self.show_delete_dialog = False
    ...
    def download_csv(self):
        ...
        output.close()
        return rx.download(data=csv_data, filename="contratos_adhesion.csv")
    ...
    def update_supplier_search(self, text: str):
        ...
        if text:
            lower = text.lower()
            self.filtered_suppliers = [
                s for s in self.adhesion_suppliers_list
