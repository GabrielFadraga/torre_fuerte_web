...
from TFuerte.api.commercial_api import SuppliersAPI

class CommercialSuppliersState(rx.State):
    # Datos
    ...
    def load_data(self):
        ...
        data = SuppliersAPI.get_all()
        if data is None:
            self.suppliers_data = []
            self.filtered_data = []
            yield rx.toast.error("No se pudieron cargar los proveedores", position="top-right")
        else:
            self.suppliers_data = data
            self.apply_filters()
            if len(data) == 0:
                yield rx.toast.info("No hay proveedores registrados", position="top-right")
            else:
                yield rx.toast.success(f"Proveedores cargados: {len(data)}", position="top-right")
    ...
    def add_supplier(self, form_data: dict):
        ...
        result = SuppliersAPI.insert(form_data)
        if result:
    ...
    def update_supplier(self, form_data: dict):
        ...
        result = SuppliersAPI.update(supplier_id, form_data)
        if result:
            yield from self.load_data()
    ...
    def delete_selected(self):
        if not self.selected_items:
            self.show_delete_dialog = False
            return
        success = SuppliersAPI.delete(self.selected_items)
        if success:
            self.selected_items = []
            yield from self.load_data()
            self.show_delete_dialog = False
    ...
    def download_csv(self):
        ...
        output.close()
        return rx.download(data=csv_data, filename="proveedores.csv")
